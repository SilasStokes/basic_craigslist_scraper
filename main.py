...
parser.add_argument('--config-path', default='myconfig.json', help='pass the file path to your keyfile')
...
def translate_html_elements():
    ...
    for el in free_elements:
        ...
        result = Craigslist_Result_Card(link=link, title=title, id=id, screenshot_path='' )
...
def scrape(url):
    ...
    with open('database.json', 'w') as db:
        json.dump(Craigslist_Result_Card.schema().dump(existing_records, many=True), db)
